...
import com.upendar.myapp.config.KafkaProperties;
...
class UserRegistrationKafkaResourceIT {
    ...
    void setup() {
        ...
        UserRegistrationKafkaResource kafkaResource = new UserRegistrationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/user-registration-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/user-registration-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
